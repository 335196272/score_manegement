...
<html>
	...
	<body>
		...
		<div class="page-container">
			<form id="searchForm" name="searchForm" action="${ROOT}/admin/classes/list.html" method="get">
				<div class="text-c">
					<input type="text" name="name" id="name" value="${param.name}" placeholder=" 班级名称关键词" style="width:150px" class="input-text">
			...
			<div class="cl pd-5 bg-1 bk-gray mt-20">
				<span class="l">
					<a class="btn btn-primary radius" href="javascript:;" onclick="add_classes()">
						<i class="Hui-iconfont Hui-iconfont-add"></i> 添加班级
					</a>
				</span>
				...
				<table class="table table-border table-bordered table-bg table-hover table-sort">
					...
					<tbody>
						<c:forEach var="entity" items="${list}">
							<tr class="text-c">
								<td>${entity.classesId}</td>
			                    <td>${entity.name}</td>
			                    <td>${entity.updateName}</td>
			                    <td><fmt:formatDate value="${entity.updateDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
			                    <td class="f-16 bk-blue">
			                    	<a title="修改" class="ml-5" href="javascript:;" onclick="edit_classes(${entity.classesId})">
			                    		<i class="Hui-iconfont Hui-iconfont-edit"></i>
			                    	</a> 
			                    	<a title="删除" class="ml-5" href="javascript:;" onclick="del_classes(this, ${entity.classesId})">
		...
		<script type="text/javascript">
			...
			function edit_classes(id) {
				layer.open({
					...
					maxmin: true,
					area: ['350px', '200px'],
					content: 'toModify.html?classesId=' + id
				});
			}
			//删除
			function del_classes(obj, id) {
				layer.confirm('确认要删除吗？', function(index) {
					$.ajax({
						type: 'GET',
						url: 'delete.html?classesId=' + id,
						dataType: 'json',
